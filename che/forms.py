...
class ChequeForm(forms.ModelForm):
    ...
    queryset_id_cheque = (
        Factura.objects.defer("id")
        .filter(Q(total_fac1__gt=0) | Q(total_fac1__lt=0))
        .defer("total_fac")
        .order_by("-id")
    )
    id_fac = forms.ModelChoiceField(
        queryset=queryset_id_cheque,
        widget=forms.Select(
...
class DepositoForm(forms.ModelForm):
    ...
    queryset_id_cheque = (
        Cheque.objects.defer("id")
        .filter(estado_che=False)
        .defer("estado_che")
        .select_related("id_fac", "proveedor", "cuenta")
        .order_by("-id")
    )
    cheque = forms.ModelChoiceField(
        queryset=queryset_id_cheque,
        widget=forms.Select(
            attrs={"class": "form-control select2", "style": "width: 100%"}
        ),
...
class CheRechazadoForm(forms.ModelForm):
    ...
    cheque_re = forms.ModelChoiceField(
        queryset=queryset_id_che,
        widget=forms.Select(
            attrs={"class": "form-control select2", "style": "width: 100%"}
        ),
    )
    ...
    cheque_nu = forms.ModelChoiceField(
        queryset=queryset_id_che,
        widget=forms.Select(
            attrs={"class": "form-control select2", "style": "width: 100%"}
        ),
    ...
    queryset_id_fac = (
        Factura.objects.filter().select_related("proveedor").order_by("-id")
    )
    id_facturas = forms.ModelChoiceField(
    ...
    class Meta:
        ...
        widgets = {
            ...
            "fecha_pagar": forms.DateInput(
                attrs={"class": "form-control", "autocomplete": "off", "readonly": True}
            ),
...
class AbonoForm(forms.ModelForm):
    ...
    id_factura = forms.ModelChoiceField(
        queryset=queryset_id_fa,
        widget=forms.Select(
            attrs={"class": "form-control select2", "style": "width: 100%"}
        ),
    )
    ...
    id_cheque = forms.ModelChoiceField(
        queryset=queryset_id_che,
        widget=forms.Select(
            attrs={"class": "form-control select2", "style": "width: 100%"}
        ),
...
class AbonoEditForm(forms.ModelForm):
    ...
    class Meta:
        ...
        exclude = ["um", "fm", "uc", "cheque_equivocado", "total", "recibo"]
...
class AbonoEquivocadoForm(forms.ModelForm):
    ...
    cheque_equivocado = forms.ModelChoiceField(
        queryset=queryset_id_che_e,
        widget=forms.Select(
            attrs={"class": "form-control select2", "style": "width: 100%"}
        ),
    )
    ...
    class Meta:
        ...
        exclude = ["um", "fm", "uc", "total", "estado_abono", "recibo"]
